...
func TestExpandCreateParamsAccumulatesUnknownDiagnostics(t *testing.T) {
	// Several unknown attributes in one plan must all surface together: the
	// missing-size diagnostic no longer short-circuits the optional-unknown
	// checks, so the user fixes everything in a single apply cycle.
	model := browserPoolModel{
		Size:      types.Int64Unknown(),
	...
	for _, want := range []path.Path{path.Root("size"), path.Root("name"), path.Root("profile_id")} {
...
func TestExpandCreateParamsRejectsUnknownNonComputedOptionalValue(t *testing.T) {
	viewportAttrs := map[string]attr.Type{
	...
	cases := []struct {
		...
		{"name", func(m *browserPoolModel) { m.Name = types.StringUnknown() }, path.Root("name")},
		...
		{"viewport", func(m *browserPoolModel) { m.Viewport = types.ObjectUnknown(viewportAttrs) }, path.Root("viewport")},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			model := browserPoolModel{Size: types.Int64Value(1)}
			tc.apply(&model)

			_, diags := expandCreateParams(context.Background(), model)
			if !diags.HasError() {
				t.Fatalf("expected diagnostics for unknown %s", tc.name)
			}
			if !hasDiagnosticPath(diags, tc.want) {
				t.Fatalf("expected diagnostic at %s, got %v", tc.want, diags)
			}
		})
	}
}

func TestExpandCreateParamsAllNullModelSendsOnlySize(t *testing.T) {
	// A model with only size set (every optional left null, not unknown) must
	// produce exactly {"size": N} — no stray fields from null attributes.
	model := browserPoolModel{Size: types.Int64Value(2)}
	...
	body := marshalSDKParams(t, params)
	want := map[string]any{"size": float64(2)}
	if !jsonEqual(t, body, want) {
		t.Fatalf("expanded SDK JSON mismatch\ngot:  %#v\nwant: %#v", body, want)
	}
}

func TestExpandCreateParamsEmptyExtensionIDsSendsEmptyArray(t *testing.T) {
	// An explicit empty extension_ids list is sent as "extensions": [] (not
	// omitted) so Terraform state round-trips [] rather than drifting to null.
	// This is intentional and load-bearing for create/read round-tripping.
	model := browserPoolModel{
		Size:         types.Int64Value(1),
		ExtensionIDs: types.ListValueMust(types.StringType, nil),
	}
	...
	body := marshalSDKParams(t, params)
	want := map[string]any{"size": float64(1), "extensions": []any{}}
	if !jsonEqual(t, body, want) {
		t.Fatalf("expanded SDK JSON mismatch\ngot:  %#v\nwant: %#v", body, want)
	}
}
