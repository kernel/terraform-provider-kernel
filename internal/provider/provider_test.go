...
func TestProviderRegistersDataSources(t *testing.T) {
	...
	if len(dataSources) != 8 {
		t.Fatalf("DataSources length = %d, want 8", len(dataSources))
	}
	...
	for _, want := range []string{"kernel_api_key", "kernel_app", "kernel_browser_pool", "kernel_deployment", "kernel_project", "kernel_profile", "kernel_proxy", "kernel_extension"} {
		if !got[want] {
