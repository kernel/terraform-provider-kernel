	...
	resp.Schema = dschema.Schema{
		...
		Attributes: map[string]dschema.Attribute{
			...
			"protocol": dschema.StringAttribute{
				Computed:            true,
				MarkdownDescription: "Proxy protocol.",
			},
		},
	}
}
...
func (d *proxyDataSource) get(ctx context.Context, projectID, id string) (proxyModel, diag.Diagnostics) {
	...
	if state.ID.ValueString() != id {
		diags.AddError(
			"Proxy ID Mismatch",
			"Kernel returned proxy "+state.ID.ValueString()+" for id selector "+id+".",
		)
		return proxyModel{}, diags
	}
	return state, diags
...
func (d *proxyDataSource) findProxiesByName(ctx context.Context, projectID, name string, diags *diag.Diagnostics) (*kernel.ProxyListResponse, int) {
	...
	for {
		...
		for _, proxy := range page.Items {
			proxyName, ok := proxyExactName(proxy, diags)
			if diags.HasError() {
				return nil, 0
			}
...
func proxyExactName(proxy kernel.ProxyListResponse, diags *diag.Diagnostics) (string, bool) {
	if !datasources.FieldPresent(proxy.JSON.Name.Raw()) {
		return "", false
	}
