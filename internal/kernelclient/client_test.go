...
func TestClientsUseProjectHeaderOnlyForProjectScopedOperations(t *testing.T) {
	t.Parallel()
	...
	if _, err := clients.GetProfile(context.Background(), "profile_123"); err != nil {
		t.Fatalf("GetProfile returned error: %v", err)
	}

	if got, want := requests[0].ProjectID, ""; got != want {
		t.Fatalf("project request header = %q, want %q", got, want)
	}
	...
	if got, want := requests[1].ProjectID, "project_123"; got != want {
		t.Fatalf("profile request header = %q, want %q", got, want)
	}
	...
	tests := map[string]struct {
		...
		"create": {
			...
			call: func(ctx context.Context, clients Clients) error {
				_, err := clients.CreateBrowserPool(ctx, kernel.BrowserPoolNewParams{Size: 1})
				return err
			},
		},
		"update": {
			...
			call: func(ctx context.Context, clients Clients) error {
				_, err := clients.UpdateBrowserPool(ctx, "pool_123", kernel.BrowserPoolUpdateParams{
					Size: kernel.Int(2),
				})
				return err
			},
		...
		"delete": {
			...
			path:   "/browser_pools/pool_123",
			call: func(ctx context.Context, clients Clients) error {
				return clients.DeleteBrowserPool(ctx, "pool_123")
			},
		},
	}
		...
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			if requests != nil {
				...
				*requests = append(*requests, capturedRequest{
					...
					ProjectID:     req.Header.Get(projectIDHeader),
