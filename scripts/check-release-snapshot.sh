...
for command in goreleaser jq unzip; do
...
# Snapshot mode checksums the renamed extra file but does not stage it in dist.
cp "$root/terraform-registry-manifest.json" "$dist/$manifest_name"
